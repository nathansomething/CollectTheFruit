...
public class CollectTheFruit extends World {
    ...
    int hitPowerup() {
        for (int i = 0; i < this.field.pUps.size(); i++) {
            if (Math.abs(this.player.loc.x - this.field.pUps.get(i).loc.x) < 15
                    && Math.abs(this.player.loc.y
                            - this.field.pUps.get(i).loc.y) < 15) {
                return i;
            }
        }
        return -1;
    ...
    public WorldImage makeImage() {
        ...
        worldImage = worldImage.overlayImages(new TextImage(new Posn(
                ...
                + (int) this.player.hunger, Color.black));
        worldImage = worldImage.overlayImages(new TextImage(new Posn(
                CollectTheFruit.width - 100, 70), "Speed: "
                + (int) this.player.speed, Color.black));
        worldImage = worldImage.overlayImages(new TextImage(new Posn(
                CollectTheFruit.width - 100, 90), "Score: "
                + (int) this.player.score, Color.black));
        return worldImage;
    }
    ...
    public WorldEnd worldEnds() {
        ...
        if (this.player.isDead()) {
            endImage = endImage.overlayImages(new TextImage(new Posn(
                    CollectTheFruit.width / 2, CollectTheFruit.height / 2),
                    "YOU LOOSE", new Black()));
